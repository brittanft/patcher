...
public class VertexBufferObject {
    ...
    private void genVertexVBO(float[][] vert, float[][] norm, float[][] textureCoordinates) {
        ByteBuffer vertices = null;//BufferUtils.createByteBuffer(vertexCount * sizeofFloat * 8);
    ...
    private void genVertexVBO(float[][] vert, float[][] norm, float[][] textureCoordinates, int[] indices) {
        genVertexVBO(vert, norm, textureCoordinates);
        ByteBuffer indicesb = null;//BufferUtils.createByteBuffer(vertexCount * sizeofInt * 1);
        for(int ind : indices){
            indicesb.putInt(ind);
        }
        indicesb.flip();
